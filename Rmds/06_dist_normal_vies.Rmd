...
## O uso da distribuição normal {.allowframebreaks}

- Como foi dito na aula passada, as __amostragens__, nos levantamentos, frequentemente são bastante __grandes__ para que as \structure{estimativas} baseadas nelas sejam mais ou menos \structure{normalmente distribuídas}.
- Além disso, com a amostragem probabilística, temos fórmulas que nos dão o valor médio e a variância das estimativas^[Também se diz "o valor médio e a variância do __estimador__".].
- Vamos considerar, primeiramente, os estimadores __sem tendência__.
