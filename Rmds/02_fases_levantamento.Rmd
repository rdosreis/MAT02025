...
\item Tirar uma amostra entre 5000 cartões perfeitamente arrumado e numerados em um arquivo é uma tarefa fácil.
\item Outra coisa é coletar amostras de habitantes de uma região onde os tranportes se fazem sobre água e através de florestas, no qual falam-se 15 línguas e dialetos diferentes, em que forasteiros ``perguntadores'' nem sempre são bem-vindos.
\end{itemize}
\end{columns}
...
- A \structure{população de estudo} é definida como a parte identificável e acessível do conjunto \structure{(população-alvo)} para o qual idealmente se deseja a informação.
...
- A __elaboração__ e o __teste__ do \structure{instrumento de coleta} de dados, acompanhado do levantamento de \structure{recursos necessários} à realização do estudo, podem ocorrer durante a construção do plano inicial.
    + \structure{recursos necessários:} contratação da equipe de campo; supervisores; treinamento (a entrevista será presencial ou por telefone?); passagens e diárias serão necessária?
...
- O tamanho da resulta da conciliação do atendimento do plano de análise proposto com as limitações dos recursos disponíveis.
...
- \structure{Codificação}, \structure{criação de variáveis derivadas} e \structure{cálculo de pesos amostrais} fazem parte da elaboração dos \structure{arquivos de dados} para análises.
    + \structure{Dicionários de variáveis} também são produzidos nesta etapa.
