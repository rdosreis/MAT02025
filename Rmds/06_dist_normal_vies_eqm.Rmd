...
- Um estimador $\hat{\theta}$ de $\theta$ dado por um plano de amostragem é denominado \structure{imparcial}\footnote{Ou {\bf não-tendencioso}, ou {\bf não-enviesado}. O {\bf viés} de um estimador $\hat{\theta}$ é definido como $\E(\hat{\theta} - \theta)$.} se o valor médio de $\hat{\theta}$, tomado em todas as amostras possíveis fornecidas pelo plano, for igual a $\theta$.
