...
- As unidades são classificadas em duas classes, \structure{$C$} e \structure{$C'$}.
- Admitiu-se uma certa \alert{margem de erro $d$} na proporção estimada \structure{$p$} de unidades na classe \structure{$C$}, e há um pequeno risco \alert{$\alpha$} de que estejamos dispostos a incorrer de que o erro real seja maior do que \structure{$d$}; ou seja, nós queremos
...
- A amostragem aleatória simples é assumida e $p$ é considerado como normalmente distribuído. Da expressão (2) da aula 17,

$$
\sigma_p = \sqrt{\frac{N - n}{N - 1}}\sqrt{\frac{PQ}{n}}.
$$

- Portanto, a fórmula que conecta \alert{$n$} com o grau de precisão desejado é
...
- Resolvendo para \alert{$n$}, encontramos
...
- Na prática, primeiro calculamos \alert{$n_0$}.
    - Se \alert{$n_0/N$} for desprezível, \alert{$n_0$} é uma aproximação satisfatória para \alert{$n$} de \eqref{n.prop}.
...
$$
em que $\alpha$ é uma pequena probabilidade.

- Assumimos que $\overline{y}$ é normalmente distribuído: do corolário 1 da aula 10, seu erro padrão é
...
- Isso geralmente é mais estável e fácil de "estimar" antecipadamente que a própria $S$.
