...
\begin{align} 
\label{vies3}
\textcolor{darkpastelpurple}{\E(\overline{y})} &= \frac{n!(N - n)!}{nN!}\times\textcolor{blue}{\frac{(N-1)!}{(n-1)!(N-n)!}\times \sum_{j=1}^N{Y_j}} \nonumber\\
&= \frac{n(n-1)!(N - n)!}{nN(N-1)!}\times\frac{(N-1)!}{(n-1)!(N-n)!}\times \sum_{j=1}^N{Y_j} \nonumber\\
&= \frac{1}{N}\sum_{j=1}^N{Y_j} = \textcolor{darkpastelpurple}{\overline{Y}}.
\end{align} 
...
\begin{align*}
\Pr(a_i = 1) &= \frac{\# \mbox{amostras que incluem o i-ésimo elemento}}{\# \mbox{amostras que podem ser sorteadas}}\\
