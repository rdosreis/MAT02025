...
- Os estimadores $\overline{y} = \sum_{i=1}^n{Y_i}/n$ e $\widehat{Y}_T = N\overline{y}$ apresentam estimativas \structure{não enviesadas} para $\overline{Y} = \sum_{i=1}^N{Y_i}/N$ e $Y_T = \sum_{i=1}^N{Y_i}$, respectivamente.
...
- Seja $t_i$ o número de vezes que a unidade $i$ aparece na amostra. Então, temos
...
- Em conjunto, as variáveis $t_i$ apresentam uma \structure{distribuição multinomial}. Por isso,
...
- Como já foi discutido anteriormente, o critério mais adotado em amostragem é o \structure{Erro Quadrático Médio}.
    + Lembre-se que quando o estimador é não enviesado, $EQM(\widehat{\theta}) = \Var(\widehat{\theta})$.
...
- Resolver os exercícios\footnote{PQP} 3.1, 3.2, 3.4a, 3.4b, 3.5, 3.6, 3.7 do Capítulo 3 do livro __Elementos de Amostagem__\footnote{Bolfarine, H. e Bussab, W. O. {\bf Elementos de Amostagem}, Blucher, 2005, p. 83-85.} (disponível no Sabi+).
