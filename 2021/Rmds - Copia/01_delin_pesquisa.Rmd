...
```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.height='80%', out.width='55%', paged.print=FALSE}

knitr::include_graphics(here::here('images', 'dor-de-cabeca-1.jpg'))
...
knitr::include_graphics(here::here('images', 'dor-de-cabeca-2.jpg'))
...
- Uma hora depois, perguntamos para cada uma delas se a dor de cabeça sumiu.
- Entre as pessoas que tomaram aspirina, a proporção de "dor de cabeça sumiu" foi 4/12; entre as que não tomaram aspirina, esta proporção foi 4/12.
- Qual conclusão podemos obter sobre a aspirina?
    + Aqui os grupos foram alocados de forma aleatória (cada indivíduo possuía probabilidade 1/2 de ser alocado ao grupo "tomar aspirina").

## Estudos observacionais {.allowframebreaks}

- Nas pesquisas em que a unidade de estudo é o ser humano (ou seres vivos), os investigadores, por questões éticas e práticas, não podem controlar a ocorrência da variável independentes ou tratamento nem proceder à alocação aleatória das unidades observadas aos grupos de comparação, optando, assim, por delineamentos considerados \structure{não experimentais (estudos observacionais)}.
    + Estudos de coorte;
...
- Algumas tomam aspirina; outras não.
- Uma hora depois, perguntamos para cada uma delas se a dor de cabeça sumiu.
- Entre as pessoas que tomaram aspirina, a proporção de "dor de cabeça sumiu" foi 1/12; entre as que não tomaram aspirina, esta proporção foi 4/12.
- Qual conclusão podemos obter sobre a aspirina?
    + Aqui os indivíduos com dor de cabeça mais grave eram mais propensos a tomar aspirina.
...
- Os levantamentos podem ter a finalidade __descritiva__, limitando-se a estimar frequências de elementos com determinada propriedade ou médias e variâncias de características quantitativas.
...
- Outros levantamentos , denominados __analíticos__ (ou investigações), definem grupos de comparação e, além de estimar, procuram detectar relações entre as características, buscando aumentar as explicações para o objeto pesquisado.
...
- Pelo mesmo motivo, os dados podem ser coletados e sintetizados mais rapidamente com uma amostragem, que com uma contagem completa. Isso é um fator primordial, quando se necessita urgentemente das informações.
